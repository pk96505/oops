...
class cur_acct:public account{
	...
	float balance_c;
public:
	cur_acct(char *name, char *type):account(name,type){
...
class sav_acct:public account{
	...
	char *withd_status_s;
public:
	sav_acct(char *name, char *type):account(name,type){
...
int main(){
	...
	float amount;
	int c=2,s=1,ch,j;
	cur_acct c_a("Amit","current");
	sav_acct s_a("Pankaj","saving");
	do{
		...
		switch(ch){
			...
			if(type==1){
				...
				cin>>account_no;
				for(j=0; j<1; j++){
					if(c_a.check_acc_no()==account_no)
					break;
				}
				c_a.deposit_amount();
				}
					...
					if(type==2){
						...
						cin>>account_no;
						for(j=0; j<1; j++){
							if(s_a.check_acc_no()==account_no)
							break;
						}
						s_a.deposit_amount();
					}
			...
			if(type==1){
				...
				switch(c_c){
				    ...
				    c_a.withd_cheque(c_a.cheque_no());
				    break;
